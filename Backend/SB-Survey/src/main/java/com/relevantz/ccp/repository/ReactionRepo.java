...
public interface ReactionRepo extends CrudRepository<Reaction, Long> {
	...
	@Query(value = "select COUNT(*)  FROM tbl_reaction WHERE survey_id=?", nativeQuery = true)
	   public long getReactionCount(long surveyId);
	
	@Modifying
	@Query(value= "delete from tbl_reaction order by reaction_id desc limit 1;", nativeQuery = true)
